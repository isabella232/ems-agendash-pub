...
module.exports = function(agenda, options) {
    ...
    agenda.on('ready', () => {
        ...
        collection.createIndexes([
            {key: {name: 1}},
            {key: {nextRunAt: -1, lastFinishedAt: -1, lastRunAt: -1}},
            {key: {name: 1, nextRunAt: -1, lastFinishedAt: -1, lastRunAt: -1}}
        ], err => {
            if (err) {
    ...
    const disableJobs = (jobIds, jobDisableReason, callback) => {
        if (!agenda) {
            return callback('Agenda instance is not ready');
        }
        try {
            ...
            .update(
                ...
                },
                {
                    $set: {
                        ...
                        disableReason: jobDisableReason
                    }
                },
                { multi: true },
                function (err, commandResult) {
